function [undersamplingPatternTxtDocPath] = convertUndersamplingPatternTxtFile(undersamplingPatternPath, oneMaskEach3DVolume, savePath)
    ...
    if (oneMaskEach3DVolume)
        fprintf("An undersampling pattern for each 3D volume will be saved in the txt file (multi mask).\n");
        
        timeNow = datevec(now);
        undersamplingPatternTxtDocPath = (strcat(savePath,num2str(timeNow(1)),num2str(timeNow(2),'%.2d'),num2str(timeNow(3),'%.2d'),'_',num2str(timeNow(4),'%.2d'),num2str(timeNow(5),'%.2d'),num2str(timeNow(6),'%02.0f'),'_UndersamplingPattern_',num2str(size(undersamplingPattern,1)),'x',num2str(size(undersamplingPattern,2)),'x',num2str(size(undersamplingPattern,3)),'_AF-',num2str(AF),'_overallAF-',num2str(actual4DAF),'_DWIAF-',num2str(correctedAF),'_nVol-',num2str(size(undersamplingPattern,5)),'_nB0-',num2str(nB0s),'_fsCenterArea-',num2str(undersamplingPatternFullySampledRegionArea),'_',undersamplingPatternType,'_',undersamplingPatternStrategy,'_',B0SamplingStrategy,'.txt'));
        ...
        fprintf("Only one undersampling patter for all 3D volumes will be saved in the txt file (mono mask).\n");
    end
